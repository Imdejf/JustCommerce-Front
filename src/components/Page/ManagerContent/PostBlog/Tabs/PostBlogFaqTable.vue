...
interface BlogFaqFormModel {
  blogItemFaqId?: string
  blogItemId?: string
  question: string
  answer: string
  displayOrder: number
...
const items = ref<BlogFaqViewModel[]>([])
const isLoading = ref(false)
const isSaving = ref(false)
const isEditing = ref(false)
const editingId = ref<string | null>(null)

const form = ref<BlogFaqFormModel>({
  blogItemFaqId: '',
  blogItemId: '',
  question: '',
  answer: '',
...
const blogItemId = computed(() => (props.postBlog as any)?.id as string | undefined)

const normalizeFaqItem = (item: any): BlogFaqViewModel => {
  return {
    id: item?.id ?? item?.Id ?? item?.blogItemFaqId ?? item?.BlogItemFaqId ?? '',
...
const startEdit = (item: BlogFaqViewModel) => {
  isEditing.value = true
  editingId.value = item.id

  Object.assign(form.value, {
    blogItemFaqId: item.id,
    blogItemId: blogItemId.value ?? '',
...
const getAllFaq = async () => {
  ...
  try {
    const result = await Api.postBlogs.getFaqByBlogItemId(blogItemId.value)
    ...
    console.error(error)
    items.value = []
    toast.error('Nie udało się pobrać FAQ wpisu blogowego.')
  } finally {
...
const saveFaq = async () => {
  ...
  try {
    if (isEditing.value && form.value.blogItemFaqId) {
      await Api.postBlogs.updateFaq({
        body: JSON.stringify({
          blogItemFaqId: form.value.blogItemFaqId,
          blogItemId: blogItemId.value,
...
<template>
  <div>
    <FormSection title="FAQ wpisu blogowego">
          ...
          <div class="flex items-center justify-between mb-6">
            <div>
              ...
              <p class="text-sm text-gray-500 mt-1">
                Uzupełnij pytanie i odpowiedź przypisaną do wpisu blogowego.
              </p>
            </div>
          ...
          <div class="space-y-5">
            <div>
              ...
              <input
                ...
                placeholder="Np. Jak długo trwa realizacja?"
                :disabled="isSaving"
              />
            </div>
        ...
        <div class="w-[58%] rounded-2xl border border-gray-200 bg-white shadow-sm p-6">
          <div class="flex items-center justify-between mb-5">
            <div>
              ...
              <p class="text-sm text-gray-500 mt-1">
                Wszystkie pytania i odpowiedzi przypisane do tego wpisu blogowego.
              </p>
            </div>
          ...
          >
            Brak wpisów FAQ dla tego wpisu blogowego.
          </div>
